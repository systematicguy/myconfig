...
$dscPublicKeyPath = $LocalConfig.DscPublicKeyPath
...
$dscCert | Export-Certificate -FilePath $dscPublicKeyPath -Force
$dscPublicKeyPath = (Resolve-Path $dscPublicKeyPath).Path
...
# Import-Certificate -FilePath $dscPublicKeyPath -CertStoreLocation Cert:\LocalMachine\My
...
Configuration LCMConfig 
{
    Node "localhost"
    {
        Settings
        {
            CertificateID = $dscCert.Thumbprint
        }
    }
}
...
$dscConfigData = @{
    AllNodes = @(
        @{
            ...
            CertificateFile     = $dscPublicKeyPath
...
$dscConfigData | ConvertTo-Expression | Out-File $dscConfigPath